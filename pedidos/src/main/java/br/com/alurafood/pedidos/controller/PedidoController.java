...
public class PedidoController {
        ...
        public String returnPort(@Value("${local.server.port}")String port){
            return String.format("Request listened by the instane on the port %s", port);
        }

        @PostMapping()
        public ResponseEntity<PedidoDto> realizaPedido(@RequestBody @Valid PedidoDto dto,
                                                       UriComponentsBuilder uriBuilder) {
            PedidoDto pedidoRealizado = service.createOrder(dto);
        ...
        public ResponseEntity<PedidoDto> atualizaStatus(@PathVariable Long id,
